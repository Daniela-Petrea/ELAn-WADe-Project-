...
with open("esoteric_languages_with_abstract_and_url.csv", newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        ...
        if 'Abstract' in row and row['Abstract']:
            g.add((language_uri, ESOLANG.abstract, rdflib.Literal(row['Abstract'], datatype=XSD.string)))

        if 'URL' in row and row['URL']:
            g.add((language_uri, ESOLANG.url, rdflib.Literal(row['URL'], datatype=XSD.anyURI)))

g.serialize("populated_ontology_with_abstract_and_url.rdf", format="xml")
print("Ontology populated and saved as 'populated_ontology_with_abstract_and_url.rdf'")
